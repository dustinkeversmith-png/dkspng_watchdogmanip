...
use crate::context::model::{
    AliasRecord, AliasTargetRef, ContextNode, CurrentObjective, InheritancePolicy, QueueItem,
    SymbolRecord,
};
...
impl Default for ParseConfig {
    fn default() -> Self {
        Self {
            default_context_id: "project".to_string(),
        }
    }
}
...
pub fn build_index_from_document(input: &str, config: ParseConfig) -> Result<ContextIndex> {
    ...
    index.upsert_context(ContextNode::new(
...
fn parse_line_into_index(line: &str, config: &ParseConfig, index: &mut ContextIndex) -> Result<()> {
    ...
    match head.to_ascii_lowercase().as_str() {
        ...
        "@current" => {
            let context_id =
                option_value(&rest, "context").unwrap_or_else(|| config.default_context_id.clone());
            let title = filtered_title(&rest, &["context"]);
            ensure_context(index, &context_id);
            index
                .context_mut(&context_id)?
                .currents
                .push(CurrentObjective {
                    title,
                    details: None,
                    source_path: None,
                });
        ...
        "@symbol" => {
            if rest.len() < 2 {
                return Err(ContextError::ParseError(
                    "@Symbol requires: <kind> <name> [context=...] [path=...] [line=...]".into(),
                ));
            }
            let kind = rest[0].to_string();
            let name = rest[1].to_string();
            let context_id = option_value(&rest[2..], "context")
                .unwrap_or_else(|| config.default_context_id.clone());
...
fn parse_context(rest: &[&str]) -> Result<ContextNode> {
    if rest.len() < 2 {
        return Err(ContextError::ParseError(
            "@Context requires: <id> <root_path> [parent=...] [inheritance=...] [imports=a,b]"
                .into(),
        ));
    }
...
fn parse_alias(rest: &[&str], config: &ParseConfig) -> Result<(String, AliasRecord)> {
    ...
    let context_id =
        option_value(&rest[2..], "context").unwrap_or_else(|| config.default_context_id.clone());
    let target_type =
        option_value(&rest[2..], "type").unwrap_or_else(|| infer_target_type(raw_target));
    let target = match target_type.as_str() {
        "folder" => AliasTargetRef::Folder {
            path: PathBuf::from(raw_target),
        },
        "symbol" => AliasTargetRef::Symbol {
            name: raw_target.to_string(),
            kind: option_value(&rest[2..], "kind"),
        },
        "context" => AliasTargetRef::Context {
            context_id: raw_target.to_string(),
        },
        "workspace" => AliasTargetRef::Workspace {
            workspace_id: raw_target.to_string(),
        },
        "command" => AliasTargetRef::Command {
            command_name: raw_target.to_string(),
            args: option_value(&rest[2..], "args")
                .map(|v| v.split(',').map(|s| s.to_string()).collect())
                .unwrap_or_default(),
        },
        "search" => AliasTargetRef::Search {
            query: raw_target.to_string(),
            scope: option_value(&rest[2..], "scope"),
        },
        _ => AliasTargetRef::File {
            path: PathBuf::from(raw_target),
            line: option_value(&rest[2..], "line").and_then(|v| v.parse::<u32>().ok()),
            column: option_value(&rest[2..], "col")
                .or_else(|| option_value(&rest[2..], "column"))
...
fn filtered_title(tokens: &[&str], option_keys: &[&str]) -> String {
    tokens
        ...
        .filter(|token| {
            !option_keys
                .iter()
                .any(|key| token.starts_with(&format!("{key}=")))
        })
        .copied()
        .collect::<Vec<_>>()
        .join(" ")
...
fn parse_inheritance(value: &str) -> Result<InheritancePolicy> {
    match value.to_ascii_lowercase().as_str() {
        ...
        other => Err(ContextError::ParseError(format!(
            "unknown inheritance policy: {other}"
        ))),
    }
}
