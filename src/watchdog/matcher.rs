...
impl CompiledWatchSpec {
    ...
    pub fn path_allowed(&self, path: &Path) -> bool {
        ...
        if let Some(exclude) = &self.exclude {
            if exclude.is_match(match_path) || exclude.is_match(path) {
                return false;
            }
        }
    ...
    pub fn matching_rules<'a>(&'a self, event: &'a FileEvent) -> Vec<(&'a WatchRule, Vec<String>)> {
        let pathless_trigger = matches!(
            event.trigger,
            WatchTrigger::Timer | WatchTrigger::Startup | WatchTrigger::Manual
        );
        if !pathless_trigger && !self.path_allowed(&event.path) {
            return vec![];
        }
        self.spec
            .rules
...
fn condition_matches(
    ...
    match condition {
        WatchCondition::PathContains { text } => {
            ...
            if ok {
                reasons.push(format!("path contains {}", text));
            }
            ok
        }
        WatchCondition::ExtensionIs { extension } => {
            ...
            if ok {
                reasons.push(format!("extension is {}", expected));
            }
            ok
        }
        WatchCondition::MetadataEquals { key, value } => {
            ...
            if ok {
                reasons.push(format!("metadata {} == {}", key, value));
            }
            ok
        }
        WatchCondition::ContextIs { context_id } => {
            ...
            if ok {
                reasons.push(format!("context is {}", context_id));
            }
            ok
        }
    }
