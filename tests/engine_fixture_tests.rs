...
fn context_file_tree_fixture_assigns_unique_context_layers_and_indexes_up_down() {
    ...
    let index = context::build_contexts_from_file_tree(
        &root,
        context::FileTreeContextOptions {
    ...
    assert_eq!(
        parser_up,
        vec!["project_src_parser", "project_src", "project"]
    );
...
fn watchdog_filters_file_types_ignores_paths_and_expands_routines() {
    let spec =
        watchdog::read_watch_spec("tests/fixtures/watch_spec_file_types_and_timer.json").unwrap();
    let events =
        watchdog::read_file_events_jsonl("tests/fixtures/file_change_events.jsonl").unwrap();

    let planned = watchdog::WatchdogPlanner::plan(&spec, &events).unwrap();
    assert!(planned
        .iter()
        .any(|a| a.event_id == "evt_rs_1" && a.rule_id == "rust_file_modified"));
    assert!(planned
        .iter()
        .any(|a| a.event_id == "evt_md_1" && a.rule_id == "markdown_file_modified"));
...
fn watchdog_timer_event_runs_timely_routine_from_fixture() {
    ...
    assert!(planned
        .iter()
        .any(|a| a.event_id == "evt_timer_due" && a.rule_id == "timer_refresh"));
...
fn parser_deeply_nested_commands_are_inserted_into_database_and_searchable() {
    ...
    assert!(
        output.commands.len() >= 8,
        "expected nested command and inferred heading/prose captures, got {}",
        output.commands.len()
    );
    assert!(output
        .commands
        .iter()
        .any(|c| matches!(c.kind, parse::CommandKind::Project)));
    assert!(output
        .commands
        .iter()
        .any(|c| matches!(c.kind, parse::CommandKind::Task)));
    assert!(output
        .commands
        .iter()
        .any(|c| matches!(c.kind, parse::CommandKind::Alias)));
    assert!(output
        .commands
        .iter()
        .any(|c| matches!(c.kind, parse::CommandKind::QA)));
    ...
    assert!(parser_hits
        .iter()
        .any(|hit| hit.source_name == "deep_nested_macros.md"));

    let boundary_hits = db.search("boundary");
    assert!(boundary_hits.iter().any(|hit| hit
        .title
        .as_deref()
        .unwrap_or_default()
        .to_ascii_lowercase()
...
fn history_log_tracks_file_navigation_explorer_locations_and_console_commands() {
    let events =
        history::read_jsonl_events("tests/fixtures/history_navigation_commands.jsonl").unwrap();
    assert_eq!(events.len(), 6);
    assert!(events
        .iter()
        .any(|e| matches!(e.target, history::HistoryTarget::File { .. })));
    assert!(events
        .iter()
        .any(|e| matches!(e.target, history::HistoryTarget::Folder { .. })));
    assert!(events
        .iter()
        .any(|e| matches!(e.target, history::HistoryTarget::Command { .. })));
    assert!(events
        .iter()
        .any(|e| matches!(e.target, history::HistoryTarget::Window { .. })));
    ...
    assert!(top
        .iter()
        .any(|stat| stat.label == "cargo test parser_database_search" && stat.total_count == 2));
...
fn history_can_append_live_style_events_with_metadata() {
    ...
    assert_eq!(
        loaded[0].metadata.get("opened_by"),
